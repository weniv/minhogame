...
function spawnZombie() {
    ...
    const headMaterial = new THREE.MeshLambertMaterial({ color: 0x27ae60 });
    ...
    const armMaterial = new THREE.MeshLambertMaterial({ color: 0x27ae60 });
...
function updateZombies(deltaTime) {
    ...
    zombies.forEach(zombie => {
        ...
        if (distance < ZOMBIE_ATTACK_RANGE) {
            ...
            // 걷기 애니메이션 (흔들림)
            const walkAnimation = Math.sin(time * 5 + zombie.userData.animationOffset) * 0.1;
            zombie.position.y = Math.abs(walkAnimation);
            zombie.rotation.x = walkAnimation * 0.1;
        }
    });
    ...
    // 웨이브의 모든 좀비가 스폰되고 모두 처치되면 다음 웨이브
    if (zombies.length === 0 &&
...
function nextWave() {
    ...
    showMessage(`웨이브 ${gameState.wave - 1} 클리어! +500원`);
...
function shoot() {
    ...
    for (let zombie of zombies) {
        ...
        if (intersects.length > 0) {
            ...
            setTimeout(() => {
                hitPart.material.color.setHex(hitPart.name === 'head' ? 0x27ae60 : 0x2ecc71);
            }, 100);
    ...
    for (let block of blocks) {
        ...
        if (blockBox.intersectsBox(playerBox)) {
            ...
            if (playerVelocity.y < 0) {
                ...
                if (feetY < blockBox.max.y && feetY > blockBox.min.y - 0.5) {
